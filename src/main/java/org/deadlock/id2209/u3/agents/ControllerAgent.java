...
import org.deadlock.id2209.u3.agents.ProfilerAgent;
...
//        container[i] = runtime.createAgentContainer(new ProfileImpl());
      final Properties auctioneers = new Properties();
      auctioneers.put(Profile.CONTAINER_NAME, "auctioneers");
      final AgentContainer auctioneerContainer = runtime.createAgentContainer(new ProfileImpl(auctioneers));

      final Properties hm = new Properties();
      hm.put(Profile.CONTAINER_NAME, "hm");
      final AgentContainer hmContainer = runtime.createAgentContainer(new ProfileImpl(hm));

      final Properties galileo = new Properties();
      galileo.put(Profile.CONTAINER_NAME, "galileo");
      final AgentContainer galileoContainer = runtime.createAgentContainer(new ProfileImpl(galileo));
//      }
      doWait(100);
      ...
      for (Object location : locations.entrySet()) {
        System.out.println("Location: " + location + ", location");
      }

      try {
        final AgentController hmprofiler = hmContainer.createNewAgent(
            "hmprofiler",
            ProfilerAgent.class.getName(),
            new Object[]{"lowbidder", 1, "hm"});
        hmprofiler.start();

        final AgentController galileoprofiler = galileoContainer.createNewAgent(
            "galileoprofiler",
            ProfilerAgent.class.getName(),
            new Object[]{"highbidder", 1, "galileo"});
        galileoprofiler.start();

        final AgentController curator = auctioneerContainer.createNewAgent(
            "curator",
            CuratorAgent.class.getName(),
            new Object[]{1, new String[] {"hm", "galileo"}});
        curator.start();
